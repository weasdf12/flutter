import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';

void main(){
  runApp(MyApp());
}
class MyApp extends StatelessWidget{
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('04.위젯키'),),
        body: WidgetKeyTest(),
      ),
    );
  }

}

class WidgetKeyTest extends StatefulWidget{
  @override
  State<StatefulWidget> createState() =>_WidgetKetTestState();
}

class _WidgetKetTestState extends State<WidgetKeyTest>{
  
  bool _swap = false;
  
  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        ElevatedButton(
            onPressed: (){
              setState(() {
                _swap = !_swap;
              });
            },
            child: const Text('순서 바꾸기')
        ),
        Row(
          children: _swap
          ? [
            CounterBox(key: ValueKey(101), color: Colors.yellow),
            CounterBox(key: ValueKey(201), color: Colors.purple)
          ]
          : [
            CounterBox(key: ValueKey(201), color: Colors.purple),
            CounterBox(key: ValueKey(101), color: Colors.yellow),
          ]
        )
      ],
    );
  }
}

class CounterBox extends StatefulWidget{

  final Color color;

  const CounterBox({super.key, required this.color});

  @override
  State<StatefulWidget> createState() => _CounterBoxState();
}

class _CounterBoxState extends State<CounterBox>{

  int _count = 0;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){
        setState(() {
          _count++;
        });
      },
      child: Container(
        width: 200,
        height: 200,
        color: widget.color,
        child: Center(
          child: Text('$_count', style: TextStyle(fontSize: 20),),
        ),
      ),
    );
  }

}